# 🎮 Focus Guard: 1-Bit Edition - Project Status
...
- [x] **AI 감시 엔진**: 실시간 '사람' 및 '스마트폰' 감지 루프.
- [x] **1-Bit Vision 필터**:
  - 사용자의 실제 모습을 1-bit 그레이스케일 스타일로 변환하여 렌더링.
  - 감지된 객체(사람, 폰)에 도트 스타일의 바운딩 박스 표시.
- [x] **스코어링 시스템**:
  - 실시간 집중 시간(Focus Time) 및 패널티 시간(Penalty Time) 추적 (HH:MM:SS 형식).
  - 집중 시(사람 감지 + 폰 미감지) 집중 시간 누적.
  - 딴짓 시(사람 미감지 혹은 폰 감지) 패널티 시간 누적 및 경고 오버레이 출력.
  - 1시간 집중 목표 대비 진행률 표시.
